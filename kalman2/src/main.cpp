...
// hier testen fuer kalman + compass
...
void pid_drive (double distance, double spd) {
    ...
    double start_angl = ((get_compass_angle() * 180) / PI) + 5;
    ...
    double out2 = 0.0;
    double setpoint2 = 0;
    PID pid_comp = PID(&error2, &out2, &setpoint2, 0.1, 0.0, 0.0, 1, reverse);
    pid_comp.SetMode(1);
    pid_comp.SetSampleTime(100);
    pid_comp.SetOutputLimits(-100, 100);
    ...
    while (fabs(getLeftDistance()) < distance && fabs(getRightDistance()) < distance) {
        ...
        if (pid.Compute()) {
            cout << "--- ERR --- OUT --- 1"/* << pid.Compute()*/ << endl;
            cout << error << endl;
            //pid.Compute();
        ...
        error2 = cmp * (-1);
        if (pid_comp.Compute()) {
            cout << "--- ERR --- OUT --- 2"/* << pid.Compute()*/ << endl;
            cout << error << endl;
            //pid.Compute();
            //cout << "Comp " << pid.Compute() << endl;
            cout << out << endl;
...
int main() {
    ...
    */
    ...
    set_tick_factor(1);
    set_round_ticks(1600, 1750); // 1600 1685 muss man direkt vor dem fahren tunen: links, rechts (wie schnell welcher ist)
